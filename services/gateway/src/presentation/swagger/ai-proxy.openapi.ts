...
export function buildAiProxyOpenApiPaths(): OpenAPIObject['paths'] {
  ...
  return {
    ...
    '/api/v1/admin/ai/providers/{code}': proxyPath(
      'patch',
      'Update AI provider priority/name/model',
      adminTag,
    ),
    ...
    '/api/v1/admin/ai/prompts/{id}/submit': proxyPath(
      'post',
    ...
    '/api/v1/admin/ai/prompts/{id}/rollback': proxyPath(
      'post',
      'Rollback prompt to prior version',
      adminTag,
    ),
    '/api/v1/admin/ai/knowledge/sources': proxyPath('get', 'List knowledge sources', adminTag),
    '/api/v1/admin/ai/usage': proxyPath(
      'get',
      'AI usage, latency, cache hit rate & estimated cost',
      adminTag,
    ),
