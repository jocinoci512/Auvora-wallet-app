...
async function bootstrap(): Promise<void> {
  ...
  app.use(
    helmet({
      contentSecurityPolicy: {
        useDefaults: true,
        directives: {
          'img-src': ["'self'", 'data:', 'https:', 'http://cdn.auvora.local'],
          'media-src': ["'self'", 'https:', 'http://cdn.auvora.local'],
          'script-src': ["'self'"],
        },
      },
    }),
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('Auvora NFT & Digital Asset Management')
    .setDescription('Multi-provider NFT discovery, collections, metadata, and gallery APIs')
