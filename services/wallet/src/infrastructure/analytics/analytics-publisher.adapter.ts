...
const DEFAULT_SOURCE_SERVICE = 'wallet';

/**
 * Thin adapter that forwards a domain "completed" event to the Analytics Platform's internal
 * ingestion endpoint (`POST /api/v1/internal/analytics/events`). Intentionally fire-and-forget:
 * an analytics-platform outage must never fail the wallet operation that triggered it.
 * No-ops when ANALYTICS_SERVICE_URL/INTERNAL_API_KEY are not configured.
 */
@Injectable()
