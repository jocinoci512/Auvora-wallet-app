...
import { AggregationWorkerService } from '../../application/services/aggregation-worker.service';
...
export class HealthController {
  ...
  constructor(
    ...
    @Optional()
    @Inject(AggregationWorkerService)
    private readonly aggregationWorker?: AggregationWorkerService,
  ) {}
  ...
  async getReady(): Promise<HealthCheckResponse> {
    ...
    const checks: Record<string, HealthStatus> = {
      ...
      aggregationWorker: workerHealthy ? HealthStatus.Ok : HealthStatus.Unhealthy,
    };
