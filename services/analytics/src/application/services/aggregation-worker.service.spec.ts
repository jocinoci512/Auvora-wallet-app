...
describe('AggregationWorkerService', () => {
  it('does not start timer in test environment', () => {
    const env = { NODE_ENV: 'test', ANALYTICS_AGGREGATION_WORKER_ENABLED: true, ANALYTICS_AGGREGATION_POLL_MS: 1000 };
    const aggregation = { processPendingEvents: jest.fn() };
    const scheduledReports = { processDueReports: jest.fn() };
    const worker = new AggregationWorkerService(env as never, aggregation as never, scheduledReports as never);
    worker.onModuleInit();
