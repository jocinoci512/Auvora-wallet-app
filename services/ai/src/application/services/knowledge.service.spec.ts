...
function buildPrismaMock() {
  ...
  return {
    ...
    aiDocument: {
      ...
      findFirst: jest
        .fn()
        .mockImplementation(({ where }: { where: { sourceId: string; title: string } }) =>
          Promise.resolve(
            [...documents.values()].find(
              (d) => d['sourceId'] === where.sourceId && d['title'] === where.title,
            ) ?? null,
          ),
        ),
      update: jest
...
describe('KnowledgeService', () => {
  ...
  it('ingests a document: chunks, embeds, and marks it INDEXED', async () => {
    ...
    const document = await service.ingestDocument('src-1', {
      title: 'Overview',
      content,
      chunkSize: 100,
      chunkOverlap: 10,
  ...
  it('marks a document FAILED and rethrows when chunking produces no content', async () => {
    ...
    await expect(
      service.ingestDocument('src-1', { title: 'Empty', content: '   ' }),
    ).rejects.toThrow('Document produced no chunks after parsing');
  });
  ...
  it('strips markdown syntax before chunking markdown documents', async () => {
    ...
    const chunkCall = (prisma.aiDocumentChunk.create as jest.Mock).mock.calls[0][0] as {
      data: { content: string };
    };
  ...
  it('ingesting a document with a duplicate title updates and reindexes the existing document instead of duplicating it', async () => {
    ...
    const second = await service.ingestDocument('src-1', {
      title: 'Overview',
