...
function buildPrismaMock() {
  const providers = new Map<string, Record<string, unknown>>([
    ['sim-default', { id: 'provider-1', code: 'sim-default', name: 'Simulator', providerType: 'SIMULATOR', priority: 10, isEnabled: true, defaultModel: 'sim-gpt', baseUrl: null }],
  ]);
  ...
  return {
    aiProvider: {
      ...
      update: jest.fn().mockImplementation(({ where, data }: { where: { code: string }; data: Record<string, unknown> }) => {
        const existing = providers.get(where.code) ?? {};
        const updated = { ...existing, ...data };
        providers.set(where.code, updated);
        return Promise.resolve(updated);
      }),
      upsert: jest.fn().mockImplementation(({ where, create, update }: { where: { code: string }; create: Record<string, unknown>; update: Record<string, unknown> }) => {
        const existing = providers.get(where.code);
        const record = existing ? { ...existing, ...update } : { id: `provider-${providers.size + 1}`, ...create };
...
describe('ModelRouterService', () => {
  ...
  it('setPriority updates priority and writes an audit record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ actorUserId: 'admin-1', details: { code: 'sim-default', priority: 5 } }),
    );
  });

  it('setPriority throws NotFoundError for an unknown provider code', async () => {
    const prisma = buildPrismaMock();
    const service = new ModelRouterService(routerMock as never, prisma as never, buildAuditMock() as never);
  ...
  it('upsertProvider creates a new provider row and writes an audit record', async () => {
    ...
    const created = await service.upsertProvider(
      { code: 'openai-2', name: 'Second OpenAI', providerType: 'OPENAI', priority: 25, defaultModel: 'gpt-4o-mini' },
      'admin-1',
    );
    expect(created).toMatchObject({ code: 'openai-2', name: 'Second OpenAI', priority: 25 });
    expect(audit.record).toHaveBeenCalledWith('ai.provider.upserted', expect.objectContaining({ actorUserId: 'admin-1' }));
  });
  ...
  it('upsertProvider updates an existing row without resetting fields the caller omitted', async () => {
    ...
    const updated = await service.upsertProvider({ code: 'sim-default', name: 'Simulator Renamed', providerType: 'SIMULATOR', priority: 10 });
    expect(updated).toMatchObject({ code: 'sim-default', name: 'Simulator Renamed', defaultModel: 'sim-gpt' });
  });
  ...
  it('setEnabled toggles a provider and writes an audit record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith('ai.provider.enabled_changed', expect.objectContaining({ actorUserId: 'admin-1' }));
  });
});
