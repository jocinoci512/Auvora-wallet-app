...
export class EmbeddingService {
  ...
  async embed(input: string[], options: { model?: string; providerCode?: string } = {}): Promise<EmbedResult> {
    const provider = await this.router.resolve(options.providerCode);
    return provider.embed({ input, model: options.model });
  }

  async embedOne(text: string, options: { model?: string; providerCode?: string } = {}): Promise<number[]> {
