...
export class DashboardService {
  ...
  async metrics() {
    const [
      ...
      this.prisma.signingRequest.count({ where: { status: { in: ['QUEUED', 'SCHEDULED', 'SIGNING'] } } }),
      this.prisma.signingRequest.count({ where: { status: 'AWAITING_APPROVAL' } }),
      this.prisma.recoveryRequest.count({ where: { status: { in: ['PENDING', 'AWAITING_APPROVAL', 'APPROVED'] } } }),
  ...
  async auditTrailForUser(userId: string, skip = 0, take = 50) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.custodyAuditRecord.findMany({ where, orderBy: { createdAt: 'desc' }, skip, take: Math.min(take, 200) }),
  ...
  async auditTrail(skip = 0, take = 50) {
    const [items, total] = await Promise.all([
      this.prisma.custodyAuditRecord.findMany({ orderBy: { createdAt: 'desc' }, skip, take: Math.min(take, 200) }),
  ...
  async policyViolations(skip = 0, take = 50) {
    const [items, total] = await Promise.all([
      this.prisma.custodyPolicyViolation.findMany({ orderBy: { createdAt: 'desc' }, skip, take: Math.min(take, 200) }),
