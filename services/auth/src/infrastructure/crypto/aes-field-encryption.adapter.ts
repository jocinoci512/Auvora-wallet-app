...
/**
 * AES-256-GCM field encryption copied from the observability adapter pattern.
 * Ciphertext format: `v1:{iv}:{tag}:{data}` (base64url). Key = SHA-256(AUTH_FIELD_ENCRYPTION_KEY).
 */
@Injectable()
export class AesFieldEncryptionAdapter implements FieldEncryptionPort {
  ...
  constructor(@Inject(ENV) env: ServiceEnv) {
    this.key = createHash('sha256').update(env.AUTH_FIELD_ENCRYPTION_KEY).digest();
  }
