...
process.env.SWAP_FIELD_ENCRYPTION_KEY =
  process.env.SWAP_FIELD_ENCRYPTION_KEY ?? 'test-swap-field-encryption-key-32chars!!';
process.env.OTEL_ENABLED = process.env.OTEL_ENABLED ?? 'false';
process.env.LOG_LEVEL = process.env.LOG_LEVEL ?? 'silent';
process.env.SWAP_WORKERS_ENABLED = process.env.SWAP_WORKERS_ENABLED ?? 'false';
process.env.SWAP_SIMULATOR_ENABLED = process.env.SWAP_SIMULATOR_ENABLED ?? 'true';
