...
describe('HealthController', () => {
  it('returns ok liveness payload', () => {
    const controller = new HealthController(
      { SERVICE_NAME: 'observability', SERVICE_VERSION: '0.1.0' } as never,
      { ping: async () => true } as never,
      { isHealthy: async () => true } as PrismaService,
    );
    const result = controller.getHealth();
    expect(result.status).toBe(HealthStatus.Ok);
    expect(result.service).toBe('observability');
    expect(typeof result.uptimeSeconds).toBe('number');
  });

  it('returns readiness with dependency and worker checks', async () => {
    const controller = new HealthController(
      { SERVICE_NAME: 'observability', SERVICE_VERSION: '0.1.0' } as never,
      { ping: async () => true } as never,
      { isHealthy: async () => true } as PrismaService,
      { status: () => ({ enabled: false, running: false }) } as never,
    );
    ...
    expect(result.checks?.alertWorker).toBe(HealthStatus.Ok);
  });
  ...
  it('works via Nest testing module', async () => {
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [
        { provide: ENV, useValue: { SERVICE_NAME: 'observability', SERVICE_VERSION: '0.1.0' } },
