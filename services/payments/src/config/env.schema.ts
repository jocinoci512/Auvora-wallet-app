...
export const envSchema = z.object({
  ...
  /** Optional. When set with INTERNAL_API_KEY, NotificationsPublisherAdapter forwards completed-payment events for downstream webhook/notification fan-out. */
  ...
  PAYMENTS_SIMULATOR_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform((value) => value === 'true'),
  SETTLEMENT_INTERVAL_MS: z.coerce.number().int().positive().default(30000),
});
...
export function loadEnv(source: NodeJS.ProcessEnv = process.env): ServiceEnv {
  ...
  if (parsed.data.NODE_ENV === 'production' && parsed.data.PAYMENTS_SIMULATOR_ENABLED) {
    throw new Error('PAYMENTS_SIMULATOR_ENABLED must be false in production');
  }
  return parsed.data;
}
