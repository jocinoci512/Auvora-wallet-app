...
**Phase:** 17 — Live Alchemy integration  
...
Blockchain Service (Nest)
        ...
        ▼
Provider Registry ──► Alchemy live providers (when configured)
                   └► Simulator providers (fallback / unsupported chains)
```
...
- EVM: `getTokenBalance(contract, holder)` — ERC-20 / BEP-20
- Solana: `getTokenBalance(mint, owner)`, `getRecentBlockhash()`
- Tron: `getTokenBalance(...)`, `estimateResources(address)`
- Bitcoin: `getUtxos(address)`

## Registration

`InfrastructureModule` builds the registry from Nest simulator providers, then **overrides** any chain that resolves an Alchemy RPC URL via `createAlchemyProviders(env)`.
