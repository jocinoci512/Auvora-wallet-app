...
**Last updated:** 2026-07-26 (Phase 15 — Enterprise Repository Audit)  
**Companions:** [`CODE_QUALITY_REPORT.md`](CODE_QUALITY_REPORT.md), [`ARCHITECTURE_AUDIT.md`](ARCHITECTURE_AUDIT.md), [`FINAL_RELEASE_CHECKLIST.md`](FINAL_RELEASE_CHECKLIST.md)
...
| RB4 | Stale Next `.next` causing docs build failure | **Mitigated** — clean before release builds |

## High priority debt

| ID | Area | Description | Effort |
|----|------|-------------|--------|
| TD-H1 | Gateway | Wire `createDownstreamProxyMiddleware` (feature-flagged) | M |
...
| TD-H8 | Auth UX | ADR or migrate JWT out of `localStorage` | M |

## Medium priority debt

| ID | Area | Description | Effort |
|----|------|-------------|--------|
| TD-M1 | Cache/Secrets | Adopt or formally mark incubating-only (READMEs added) | M |
...
| TD-M9 | Analytics | Aggregation job list / metric detail endpoints (SDK stubs) | M |
| TD-M10 | Tooling | Local deployment validation requires helm/terraform (CI OK) | S |
...
| TD-L7 | Jest | Migrate ts-jest `isolatedModules` config | S |
...
## Paid-down this Phase 15 pass

- Misleading AI vector-search documentation corrected  
- Incubating READMEs for `@auvora/cache` / `@auvora/secrets`  
- Stale Next build cache cleared; docs build verified  
- Full audit report suite published  
...
- Always `clean` Next `.next` dirs before release candidate image builds.
